...
export class ProductService {
	...
	getById(productId: number): Observable<Product> {
		return this.http
			...
			.pipe(map((products) => <Product>products.find((p) => p.id === productId)));
	}

	getByCategory(category: string): Observable<Product[]> {
		return this.http
			.get<Product[]>('/data/products.json')
			.pipe(map((products) => products.filter((p) => p.categories.includes(category))));
	}
 ...
 */
	...
	private filterProducts(products: Product[], params: ProductSearchParams): Product[] {
		return products
			.filter(
				(p) => (params.title ? p.title.toLowerCase().includes((<string>params.title).toLowerCase()) : products)
			)
